...
- **The install is a Helm chart** — `helm install mast oci://ghcr.io/go-steer/charts/mast --set gcp.projectID=…`, plus `scripts/setup-wif.sh` for the GCP-side IAM. Release binaries, the container image and the chart are signed with keyless cosign and carry a SLSA build provenance attestation naming the commit and workflow they were built from. There is a Terraform module for the whole GKE install in [`examples/deploy/terraform/`](./examples/deploy/terraform/) — both halves in one `apply`, and the only form that can *narrow* an IAM grant it previously widened. There is still no Homebrew tap and no apt repo ([#342](https://github.com/go-steer/mast/issues/342)).
- **Run one replica for anything scheduled.** Extra replicas no longer duplicate the work — one instance takes a lease over the session store and drives the schedules, timed resumes and boot auto-resume, and the others log at `ERROR` that they are not — but they do not share it either: a passive replica never takes over, so a restart of the pod that died is what brings the cadence back ([#345](https://github.com/go-steer/mast/issues/345), takeover open as [#403](https://github.com/go-steer/mast/issues/403)).
