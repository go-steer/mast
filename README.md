...
- **The install is a Helm chart** — `helm install mast oci://ghcr.io/go-steer/charts/mast --set gcp.projectID=…`, plus `scripts/setup-wif.sh` for the GCP-side IAM. Release binaries, the container image and the chart are signed with keyless cosign. There is still no Terraform module and no Homebrew tap ([#342](https://github.com/go-steer/mast/issues/342)).
