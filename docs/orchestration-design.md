...
> **Status check, 2026-09-14 ([#305](https://github.com/go-steer/mast/issues/305)).** Classifier-first bundle selection **is not implemented**, and neither is any of the four constraints below: `allowed_bundles` appears in no Go file and no YAML schema in this repo. The constraints are correct and they are prerequisites, not follow-ups — but calling them "mandatory" while nothing enforces them is the shape [#300](https://github.com/go-steer/mast/issues/300) catalogued, so this note says so rather than leaving a reader to assume there is a control here. Recorded in [`./threat-model.md`](./threat-model.md) §5.8.
...
**One classification consequence worth stating plainly.** *(Rewritten 2026-09-20, [#447](https://github.com/go-steer/mast/issues/447) — the original claim is below, struck, because it was the load-bearing reason nobody looked.)* ~~Because ADK's mcptoolset drops MCP annotations (so `readOnlyHint` never arrives) and the predicate is default-deny-unknown, `tool_catalog.tools[].mutating: false` is in practice the *only* way to declare an MCP read tool safe.~~ The drop is real; the conclusion was not. ADK drops annotations one layer *above* the MCP client, after the `tools/list` response has already passed through mast's own sending middleware with them attached — so mast reads the hint there and clause (b) of the predicate now does what it always said it did. What survives: **a bundle that classifies nothing is at the mercy of how well its servers annotate themselves.** A server that publishes no hints still yields an agent that parks for approval on `get_k8s_logs`, and, since W2.4, one whose read-only specialists cannot be granted the catalog at all. `tool_catalog.tools[].mutating` is no longer the only way to classify a read tool, but it is still the only *auditable* one, and it is the only one that can pin a tool against its server's word. Classify the catalog.

### Prior-state capture: a recorded effect carries a route back (2026-09-05, [#296](https://github.com/go-steer/mast/issues/296))

Everything above is about whether a change happens. This is about what is true after it has. Through v0.6 the sequence mast supported was *propose → approve → act → record that it acted*; the sequence an operator needs is *capture prior state → act → re-observe → keep or restore*, and the first and last steps were both missing. An unattended agent that can act and cannot undo puts the operator in the worst of the three available positions: they can approve a change, watch it make things worse, and have no path back except reconstructing the old state by hand under time pressure.

**The scope is capture, and it stops deliberately short of restore.** mast records the prior state before a mutating call and records the call that would put it back. It does not fire that call. Three rules bound this, and each is a refusal to guess:

1. **mast cannot derive the read.** Which tool reads the target of `patch_k8s_resource`, and with which arguments, is domain knowledge; the domain-neutrality rule that keeps a k8s schema out of mast (see ack routing below) applies here too. The workload declares it.
2. **mast cannot derive the inverse.** An inverse is not a property of a tool call — `scale_deployment` inverts by re-scaling, `delete_pod` does not invert at all, and `patch_k8s_resource` inverts only for the fields the patch touched. The workload declares that too, or declares nothing and gets a record with no undo, which is a complete declaration of a different shape rather than a broken one.
3. **mast does not fire the inverse.** The recorded revert is a `ProposedChange`, and running it sends it through this same gate. An automated rollback is a mutating call nobody approved, decided by the component whose whole job is to not do that.

**Where it happens is the same moment the precondition read already happens.** The gate is holding the call, the arguments are the model's, and nothing has reached the cluster yet — so the capture reuses `runOwnBehalf`'s existing `read` caller ([`../cmd/mast/toolschemas.go`](../cmd/mast/toolschemas.go)) rather than becoming a fourth fenced caller, and the whole capture path runs *before* the forward call. That ordering is what makes it fail-closed at no cost: when the read fails, or the declaration cannot be honored, the call is refused and nothing has happened yet. The alternative — act, then discover the record could not be taken — is the one outcome worth avoiding, because it is the state where the operator believes there is an undo and there is not.

**Three `permit` call sites cover four paths, and the count is the point.** A mutating call reaches the tool through `apply`, through an approved verdict, through an edited verdict, and through a change-set grant being spent. Approve and edit converge (`honorVerdict`), so three sites suffice; a fourth path added later without a fourth capture is a hole that no aggregate test would notice, which is why the tests assert each path separately rather than "some call was captured". The edited path captures against the arguments the *operator* ran, for the same reason `AppliedEdit` exists at all. The grant path captures *before* the grant is marked consumed, so a refused capture does not silently burn a grant.

**The record rides the state delta, which is the effect journal.** `CaptureRecord` — the change's call key and arguments, the read and its arguments, the prior values, the fields kept, a digest of the whole read, and the proposed revert — is written under `mast_effect_capture_<fcID>` onto the event ADK is already appending, exactly as `AppliedEdit`, `Decision` and `Grant` are. Since the session event log *is* the outbox ([`../DESIGN.md`](../DESIGN.md), `pkg/effects`), this is the effect journal and not a companion store: same session, same write lease, no second writer. `mast sessions show` prints what was overwritten, the tool and arguments that restore it, and — in as many words — that the undo is a proposal rather than something mast already did.

The one place the projection differs from its neighbours: an undecodable capture row comes back as a **stub** naming the function call id, where an undecodable `AppliedEdit` is skipped. They are losing different things. A dropped edit row costs audit detail about a call the transcript still shows; a dropped capture row makes the view say "nothing was captured", and an operator who reads that concludes the change is irreversible and stops looking.

**Four things are refused before run time.** The loader refuses a capture whose read is the changing tool itself (by the time it has run there is no prior state to read), a revert that names no call, a revert whose arguments all come from the change (it re-applies the change rather than undoing it — the most dangerous shape, because it is close enough to an undo to be believed during an incident), and a revert drawing on a path the declaration's `fields` does not keep. Composition refuses two more that need the catalog's classifications: a **mutating capture read**, which would write to the cluster before every gated call, unapproved and unrecorded, in the name of making the change reversible; and a **read-only revert**, which would be offered to an operator as a way back and do nothing. A deployment that declares captures but has no seam to run a read still starts — the declaring tools' calls are refused one at a time, with the reason — but says so at startup, because the failure this closes is a bundle author seeing a clean start and believing changes are reversible.

**What this leaves for later.** Restore stays out: given a record, mast can render the call but not decide that firing it an hour later is still right. Captures are per-call, so a change set is a list of records rather than one transaction with one inverse. And a capture proves the *record* exists, not that the revert would work — nothing here executes a revert to check, and a workload that wants that assurance has to exercise it in an eval.
...
| AG-UI ([`./ag-ui-design.md`](./ag-ui-design.md)) | Bundle field `agui.expose` opts workload into AG-UI publication (endpoint path + input schema); `agui.session_model` picks per-thread vs. per-run mapping; two publication opt-ins, both off/empty by default and both silent rather than redacted when off — `agui.state_projection` allowlists the state keys projected as `StateDelta` events, and `agui.emit_reasoning` publishes the model's thinking as `REASONING_*` frames. *(Corrected 2026-09-16: this cell previously read "streaming/activity/reasoning event opt-ins", none of which were fields. Streaming is not a bundle key — the run endpoint is SSE unconditionally; activity events are still deferred; reasoning shipped under [#98](https://github.com/go-steer/mast/issues/98) Stage 4 with the name above. Since [#302](https://github.com/go-steer/mast/issues/302) an unrecognised `agui:` key is a load error, so a cell that names a key is a claim a bundle can be tested against.)* | External AG-UI agents in planner vocabulary via `invoke_remote_agent("agui://...", ...)` | AG-UI usage patterns (thread lifetime, interrupt frequency, tool-call rendering) feed bundle-learning |
