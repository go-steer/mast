...
| `tool_catalog.tools[].mutating` | bool | Overrides the tool's mutation classification for the recorded-effect outbox. This override is read **before** every other source, including an MCP server's own `readOnlyHint`, so it is how you pin a tool your server classifies differently. Below it, an MCP tool is read-only if its server says so and **mutating** otherwise — including when the server publishes no annotation, which is indistinguishable on the wire from publishing `false`. Omitted means no override. Every applied override is audit-logged at startup. |
...
| `a2a.auth.required`, `a2a.auth.scopes` | bool, list of strings | Per-skill auth policy. `scopes` are enforced per call when a token validator is configured (`MAST_A2A_TOKEN`): a caller whose token lacks a scope is refused `403`. **`MAST_A2A_TOKEN` is one token carrying every exposed skill's scopes**, so that `403` needs a host-supplied validator to become reachable — see [scopes and the shared token](/reference/cli/#scopes-and-the-shared-token). |
...
| `agui.auth.required`, `agui.auth.scopes` | bool, list of strings | Per-endpoint auth policy. `scopes` are enforced per run when a token validator is configured (`MAST_AGUI_TOKEN`): a caller whose token lacks a scope is refused `403`. The list is also published on the discovery descriptor, where it states **this workload's requirement** — not a property of any token mast issues. `MAST_AGUI_TOKEN` is one token holding every exposed workload's scopes, so on a daemon configured that way the `403` is unreachable: see [scopes and the shared token](/reference/cli/#scopes-and-the-shared-token). |
