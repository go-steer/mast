...
| `--listen` | `:7777` | HTTP bind address for `/inject`, `/resume`, `/abort`, `/monitor-ack`, `/metrics`, `/healthz` (readiness; unauthenticated — see [Asking whether it is ready](/concepts/interop/#asking-whether-it-is-ready)). Non-loopback binds are refused without `MAST_INJECT_TOKEN` (`/resume` releases parked mutating calls; `/stop` stops the daemon) — so the default bind needs either the token or `--listen=127.0.0.1:7777`. A `MAST_INJECT_USERS_FILE` table does not satisfy this: it gates two routes, not the listener. |
| `--attach-listen` | (empty) | Operator attach surface (HTTP/SSE for [mast-web](https://github.com/go-steer/mast-web) and other attach clients): a TCP address like `127.0.0.1:8484`, or a Unix socket as `unix:/path/mast.sock`. Empty = disabled. **Implies a durable `--session-db` at `~/.mast/sessions.db` when you name none** — the live tail pumps from the eventlog overlay, which an in-memory store does not have, so attach used to refuse to start rather than imply one. Naming a path still wins, and `--session-db=` (explicitly empty) is refused rather than quietly overridden. The implication is sqlite-only: `--session-db-driver=postgres` still needs a DSN, because one cannot be invented. Non-loopback TCP binds are refused unless auth is configured — set `MAST_ATTACH_TOKEN`. Serve mode only. |
