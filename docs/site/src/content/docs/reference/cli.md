...
| `--dispatch` | `coordinator` | Dispatch shape: `coordinator` (SubAgents pattern), `graph` (workflow-graph LLM-as-router), or `fanout` (whole roster investigates concurrently, one `_synthesis` specialist merges). A bundle can declare its own `dispatch:`; the flag wins only when the operator actually typed it. `fanout` requires a read-only roster — see [the bundle reference](/reference/workload-bundle/). |
