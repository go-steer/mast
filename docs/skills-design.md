...
> **Implementation status, 2026-08-20: none of this has shipped.** The phasing table below puts the loader in **v0.1**; v0.1 through v0.4 all shipped without it, and there is no `pkg/skills`, no `invoke_skill`, no `mast skills` CLI, and no `.agents/skills/` discovery in the tree. What *does* exist is three declarations that read as though it did — `ToolAllowlist.Skills` in a specialist spec, `attach.ToolSourceSkill` on the `/tools` wire, and the `"skill"` plan-exempt namespace in `pkg/permissions` — none of which any production code acts on. As of #211 the first of those refuses a non-empty list at load rather than accepting a grant it cannot narrow, and the other two are annotated in place; see [`./specialists-design.md`](./specialists-design.md#the-skills-axis-does-not-exist-yet-2026-08-20). Nothing user-facing promised skills, so this is a corpus-vs-code gap rather than a regression — but read the phasing table as a plan that slipped, not as a record of what runs.
...
1. **SKILL.md format version tracking.** The format is evolving; mast should support a version range. Bias: track the spec version mast tests against per release; document explicitly; version-negotiate on load if the bundle declares a `schema_version` field. *(2026-09-11: the bundle does declare one now — [#302](https://github.com/go-steer/mast/issues/302), currently `1`, absent means 1. Note the precedent set alongside it: specialist frontmatter deliberately gets **no** version of its own, because it is only reachable through a bundle that names it. A skill loaded the same way inherits the same argument; a skill fetched from a registry, which arrives without a bundle around it, does not — that is the distinction this question turns on.)*
