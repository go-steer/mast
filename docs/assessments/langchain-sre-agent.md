...
2. **`EventTypeInteractive`** handling; map `block_actions` → mast's resume endpoint (`pkg/inject`'s `ResumeRequest` takes session + interrupt or a token) and → the ack endpoint. Per Q1's now-resolved sub-decision, acks traverse mast for authn and audit and are forwarded to lookout, which owns the state — so switchboard keeps a single backend and the approver-attribution story does not split across two systems. Approve/Reject and Ack are separate paths behind one backend, not one path: the first resumes an interrupt, the second suppresses a finding.
...
**Dataset integrity (checked 2026-08-12, before porting).** Two things a port has to know, neither of which changes the verdict that the suite is worth copying:

- `evals/` ships the same 31 scenarios twice — `sre-agent-k8s-eval.json` and `.jsonl` — and they have drifted apart on `expected_tools` for **15 of 31**. The `.jsonl` is authoritative: byte-equivalent to `create_dataset.py`'s `EXAMPLES`, and the copy the README names for LangSmith upload.
- Seven of the authoritative copy's 23 distinct tool names don't exist in the 63-name registry in `tools/__init__.py` — mostly singular/plural slips (`kubectl_get_service` vs. registered `kubectl_get_services`; `kubectl_get_pvcs` vs. `kubectl_get_pvc`) plus a `describe_*` family that was never implemented. That is 16 of 71 tool references, ~23%, which no run can satisfy, so `tool_coverage` has a ceiling well below 1.0 on their own harness.
