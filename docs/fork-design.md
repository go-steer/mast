...
3. **Issues #158/#159/#160 are demoted from the trigger.** They land in core-agent because core-agent wants them, on core-agent's schedule. Mast does not wait: the bash search-gate (#158) and tool-catalog defaults (#160) are applied at port time regardless of whether core-agent has landed them, and the watchdog→model routing (#159) is *built fresh* in bucket 3 as the v2-native emitting-node shape this doc already calls cleaner than the v1 transport. (#161's probe is closed.) *Superseded 2026-08-17 for #159:* core-agent landed the routing first (`e42a511`), so mast ported it rather than rebuilding it, and it is reclassified as shared infrastructure — see "Sync discipline under (E)" below. The v2-native adaptation happened anyway, at port time, which is where this doc expected the adaptation cost to land.
...
**Staged-gate refinement (2026-07-27).** Core-agent's cleanup progressed fast: the two structural restructures the gate most feared — #388 (`pkg/agent` god-package split) and #386 (`pkg/compose` extraction) — are **closed**, as is #384 (peer-endpoint credential exfiltration). What remains open across the three code milestones is bug-fix-shaped: #357/#367 (Anthropic adapter), #370 (Vertex cache), #363 (watchdog), #372 (low-sev roundup), #385 (attach security roundup, low), #390 (attach public API). The all-or-nothing gate therefore refines into per-package stages, applying the same "don't port moving code" logic at package granularity:

- **P1.3a — port now:** the ADK-independent packages no remaining issue touches: `pkg/permissions`, `pkg/pricing`, `pkg/digest`, `pkg/taskclass`, `pkg/instruction`, `pkg/modeltier`. Unblocks exit criterion 2 (`--task` profiles) immediately.
- **P1.3b — gate on the correctness bugs (#357/#367/#370/#363/#372):** `pkg/providers` and the watchdog surface — the open bugs live in exactly those files.
- **P1.3c — gate on #390 + #385:** `pkg/attach` — porting before the public-API cleanup means porting a shape core-agent is about to change. Unblocks exit criterion 4 (mast-web reachability) when it lands.

Per-stage attribution SHAs are pinned at each stage's port time. The #372/#385 "roundup" catch-alls are worth a scope check with the core-agent team — if their contents don't touch ported files, they shouldn't hold P1.3b/c hostage.

**P1.3b landed (2026-07-29).** Core-agent closed all four cleanup milestones on 2026-07-28 (every issue named in the stage gates included), opening P1.3b and P1.3c simultaneously. P1.3b is pinned at `go-steer/core-agent@b8dd225e9ae7fdeb3ff23772cc5be25eed34b818` and ships `pkg/providers/{anthropic,gemini,mock,vertexcache}` + `pkg/watchdog`. Three shape notes recorded for the next porter:

- Core-agent's `pkg/models` Provider/`Resolve`/registry layer was **not** ported: its `*config.Config` constructor coupling, `init()`-side-effect registration, and direct env reads are exactly what core-agent's own #492 (item 7) plans to break before the v2 freeze. Mast providers expose per-provider Options structs; backend/model dispatch lives in `internal/compose.BuildModel` (explicit switch, no registry).
- The #370 fix lives in core-agent's `internal/vertexcache`, not `pkg/models` — it ports to `pkg/providers/vertexcache` (public) so compose and library embedders can wire the cache hooks.
- The #363 fix lives in core-agent's `pkg/agent` bridge, not `pkg/watchdog` — porting the package alone would silently reintroduce the double-observe bug. Mast carries the bridge as `pkg/watchdog`'s `ObserveEvent`/`Tap` free functions; alerts are logged only (the #159-style model routing stays bucket 3, as §trigger note 3 already records).

P1.3c is unblocked by the letter of its gate (#390/#385 closed), but attach kept absorbing fixes after the milestones closed (#494–#500 on 07-28/29; #490/#491 open; #492's pre-freeze polish touches attachadapter). Same "don't port moving code" logic: pin P1.3c's SHA once attach quiets.

**P1.3c landed (2026-07-29).** Attach went quiet after the #494–#508 hardening run; the pin is `go-steer/core-agent@25d8531cf8d1d69459471009a9e7e2e9b0dff1e2` (HEAD at port time), chosen deliberately to include #519's transport-neutral `OperatorEventTarget` seam — mast never carries the deprecated attach-named emitter. *(Corrected 2026-09-14: it did. `EmitTarget`, with its `SetAttachEmitter` method and its `// Deprecated:` marker, came across with the port and sat in `pkg/attach/events.go` through v0.8, probed as a fallback and pinned by a test. What the pin bought was that `OperatorEventTarget` was always the primary name here, so no mast release ever needed the fallback — the only program it could have served is one implementing core-agent's old method against mast's attach package. It was removed under [`./compatibility-policy.md`](./compatibility-policy.md) ([#304](https://github.com/go-steer/mast/issues/304)), which is the policy's first application: an inherited deprecation with no end date is not a cycle.)* Ships `pkg/attach` + `pkg/auth` + `pkg/eventlog` + the mast-native `pkg/attachadapter`, wired behind serve mode's `--attach-listen`. Shape notes for the record:

- **The eventlog "decide at port time" question resolved itself upstream.** Core-agent's own #507-era eventlog already has exactly the re-scoped shape this doc's P1.3 table asked for: ADK's `session/database` owns the sessions/events/state tables and the package adds a thin seq-overlay table + `Since`/`Watch` stream. The port is near-verbatim with the ADK import at `/v2` (whose new event fields — `NodeInfo`, `Routes`, `RequestedInput`, `Output` — persist via ADK's own schema and ride the SSE `agent` frames for free). With `--attach-listen`, serve opens the session store through `eventlog.Open`; without it, the plain `session/database` path is unchanged.
- **`attachadapter` is reshaped, not ported.** Core-agent's adapter delegates to its persistent agent loop (inbox, wake signal, usage tracker); mast's daemon runs turns on demand against one ADK runner. The mast adapter owns inbox semantics itself — one injected message = one serialized `RunTurn`, typed operator events (`status-update` / `turn-complete` / `turn-error`) emitted in spec order around each turn, interrupt = cancel the in-flight turn's context, callers ride the turn context (`auth.WithCaller`) into eventlog metadata. Session resume-after-restart needs no ACL store in v0.1: every durable session runs against the daemon's one runner, so the resumer re-materializes an adapter from the session store alone (`/sessions/<sid>` after restart lazily resumes; unknown sessions 404).
- **Attach runs single-user in v0.1** (no multi-session auth, no ACL store, no SessionFactory — `POST /sessions` honestly 501s); bearer auth via `MAST_ATTACH_TOKEN`, and the ported loopback-only-without-auth refusal (#376) applies as-is.

**Exit criterion 4 verified same day with the real client:** mast-web served in proxy mode against a live `mast --attach-listen=…` daemon, driven headless — connect, session list, and a prompt round-tripped through a real turn over SSE (composer → attach inject → adapter turn → eventlog → live tail → transcript). The repoint surfaced one mast-web bug (its sessions-list parser followed its own mock's invented field names instead of the `{app, user, sessionID}` wire shape core-agent and mast actually emit) — fixed on the mast-web side, which is the correct side: mast matches core-agent's attach protocol byte-for-byte, per the "attach-mode-compatible" contract.
...
- **Watchdog → model context routing (issue #159)** — the emitting-node integration point is scaffolded in bucket 1; the concrete watchdog signal-emission lands in Phase 2 as bucket 3 work. *Shipped 2026-08-17 as `--watchdog=feedback`, ported from core-agent `e42a511` rather than built fresh — the observation is prepended to the session's next prompt at the `runTurnPre` chokepoint, which is where mast's turns funnel, rather than emitted from a node.*
...
| **Lean-fork-specific feature** (workflow scaffolding, audit-derived memory, opinionated task profiles, classifier-first dispatch on task class) | Lean fork only. Not ported to core-agent unless someone there explicitly asks for it. |
...
*Reclassified 2026-08-17: watchdog→model routing is shared infrastructure, not lean-fork-specific.* This table used to name it as the example in the lean-fork row, on the theory that routing a behavioral alert into the model's own context was a thing only an unattended workload wanted. The first triage found upstream had shipped it (`e42a511`, core-agent #678) along with six other watchdog commits, which is the empirical answer to a classification question: a feature both repos independently want is shared by definition, whatever the table says. mast ported all seven, adapting each to its own shape (no `Agent` struct, three signals where upstream has one, a `runTurnPre` chokepoint where upstream has an agent preflight) — see [`./sibling-sync.md`](./sibling-sync.md).

The generalizable part is not about the watchdog. **A classification is a prediction, and predictions get checked.** The lean-fork row is populated by guessing what core-agent's audience will never ask for, and the fork was young enough when this table was written that some of those guesses were bound to be wrong. When one is, reclassify — the cost of leaving a wrong row in place is a feature that lands twice, in two shapes, and diverges silently. Classifier-first dispatch on task class replaces it as the example, being a thing that follows from mast's dispatch model rather than from a guess about someone else's roadmap.
...
*Status (2026-08-17):* [`./sibling-sync.md`](./sibling-sync.md) now exists — but not as the hand-maintained per-commit ledger described above, which went unwritten for three weeks and would have gone on being unwritten. The split that works: the machine keeps the **inventory**, the doc keeps the **verdicts**. The per-file `// Originally derived from go-steer/core-agent@<SHA>` trailers turned out to be the record that actually exists — 182 of them, because they're written at port time by the person doing the port rather than remembered afterwards. `dev/upstream-drift` reads them and answers the question the doc was meant to answer: for each ported file, which upstream commits have touched its source path since the SHA it was frozen at. Two things make that a git-log query rather than a diff — every ported file differs from upstream by construction (the trailer itself, `~/.mast` paths, mast-flavored comments), so a diff is 100% noise; and an upstream *rename* makes `git log -- <old-path>` return zero commits, which reads as "in sync" unless it's reported as its own `missing-path` status. Trailers may carry an explicit `:<upstream-path>` suffix when the file was renamed on the way in. `.github/workflows/upstream-drift.yml` runs it weekly into one long-lived tracking issue; it deliberately never fails a build, because drift is the expected state here and a red build nobody can turn green trains people to ignore red builds.

One rule the first triage added, because it is the difference between the trailers being a record and being noise: **a re-port bumps the trailer.** A file updated against newer upstream while still carrying its original port-batch SHA reports absorbed drift every week forever, and a report with known-false rows in it is a report nobody reads.
...
*How that condition gets observed (added 2026-08-17):* "stabilized enough" was written without a way to tell. The weekly drift report is that instrument — per-package commit counts, sampled every Monday, so the trend is visible rather than recalled. A package whose count stays near zero across months is a stable shared surface and an extraction candidate; one that keeps moving is neither. The baseline at the report's first run: 48 upstream commits across 53 of 182 ported files in 17 packages. Note also what the report *doesn't* measure — module-level divergence. mast is `github.com/go-steer/mast` on ADK v2; core-agent is `github.com/go-steer/core-agent/v2` on ADK v1. Any extracted module that touches ADK, genai, or the MCP SDK forces both repos into version lockstep, so the plausible first slice is the code with no third-party imports at all (`pkg/pricing`, `pkg/modeltier`, `pkg/taskclass` are stdlib-only today). Extraction also needs core-agent to agree to consume it; a shared module with one consumer is a repo split, not a shared library.
