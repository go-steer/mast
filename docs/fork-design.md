...
**P1.4 — specialists loader + workload bundles + durability primitive + A2A + AG-UI + federation scaffolding + minimum bucket 3.** The `pkg/specialists/` package per [`./specialists-design.md`](./specialists-design.md), loading `.tmpl` files, registering as Task-mode (or SingleTurn-mode per `mode:` frontmatter) `LlmAgent`s via `agenttool`. The `pkg/workloads/` package per [`./orchestration-design.md`](./orchestration-design.md) — bundle schema, loader, `--workload=<name>` CLI flag, envelope routing, basic classifier-first dispatch. The `pkg/session/` durability surface per [`./durable-execution-design.md`](./durable-execution-design.md) — `Pause`/`Resume`/`List`/`Get` API against SQLite session store; CLI `mast sessions list/show/resume/abort`. The `pkg/a2a/` package per [`./a2a-design.md`](./a2a-design.md) — synchronous A2A server (`/.well-known/agent-card.json`, `/a2a/tasks/*`), synchronous A2A client, static `.agents/a2a/*.yaml` config. The `pkg/agui/` package per [`./ag-ui-design.md`](./ag-ui-design.md) — AG-UI server (per-workload HTTP+SSE endpoints; RunAgentInput acceptance; core event streaming; HITL via `RunFinished{interrupt}` + resume), AG-UI client wrapping the community Go SDK, shared `TokenValidator` with A2A. The `pkg/federation/` package per [`./federation-design.md`](./federation-design.md) — adapter interface, A2A + AG-UI adapters, planner `invoke_remote_agent` tool. Planner scaffolded (tool vocabulary schema in place; `run_shape_*` tools wire to whatever reference-graph shapes are shipped in P1.5); learning explicitly deferred to v0.3. ~9-12 days.
...
11. AG-UI round-trip: sample bundle marked `agui.expose: true` accepts a `RunAgentInput` POST; streams standard AG-UI events; HITL interrupt via `RunFinished{interrupt}` + `RunAgentInput.resume` round-trips against a stub AG-UI client using the community Go SDK.
...
9. **AG-UI activity events + push notifications + Slack-via-CopilotKit deployment starter** per [`./ag-ui-design.md`](./ag-ui-design.md) v0.2 phasing — workflow-shape visibility to AG-UI clients; long-running run resilience for chat-platform bots; `examples/deploy/slack-via-copilotkit/` starter.
