...
- Missing keys resolve to zero values (bias: fail-tolerant over fail-fast for missing memory — reducers may not have run yet).
...
Reducer execution is idempotent — running a reducer twice on the same event stream produces the same output. This enables re-derivation on schema changes, replay for debugging, and multi-instance safety (multiple mast pods running reducers race but converge).
