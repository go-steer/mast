...
5. **Reasoning event exposure.** Some model reasoning tokens are sensitive (chain-of-thought reveals prompt-injection surface). Bias: default off; opt-in per bundle (`agui.emit_reasoning: true`); document the tradeoff. *Still open — but the floor it opts in FROM is now built (#370, 2026-09-15). "Default off" was not a default: eight surfaces read model text with `part.Text != ""`, which does not exclude a thinking block, and the only reason nothing leaked was that `claude-opus-5` under the request mast sends today returns a thinking block with an empty body. Every one of those reads now goes through `internal/modeltext.Text`. What this changes about the OQ is its shape: `emit_reasoning: true` has to become a deliberate read of `Thought` parts at the AG-UI emitter, not the removal of a filter.*
