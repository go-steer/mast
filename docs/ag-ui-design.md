...
- **Deferred (documented, follow-on stages).** Sequenced 2026-09-17 (#98) against the [v1.0 API freeze](../DESIGN.md#the-v10-stability-promise), because the deciding question for each is not "how big is it" but **"does it need a `workload.Bundle` key, and does it hold a defect today"**. A key is freeze-exposed; a defect is not a feature and does not wait for one.
  ...
  |---|---|---|
  | **Client-disconnect cancellation** ([#383](https://github.com/go-steer/mast/issues/383)) | **Pre-v1.0** | Not a feature. A transport blip destroys the run, and a retry mid-tool re-fires a mutating call that already happened — measured. Mostly the removal of a coupling; see [Long-running runs](#long-running-runs). |
  | **Concurrent-run policy** ([#384](https://github.com/go-steer/mast/issues/384), [OQ #4](#open-questions)) | **Key shape pre-v1.0**, implementation may slip | The recorded bias is per-bundle, and that key lands on the frozen `Bundle`. The current behaviour — an unbounded wait ending in a wallclock timeout — is wrong but not destructive. |
...
- **Client disconnect resilience** *(mast extension — AG-UI defines no run-reattach semantics)*: ~~if the SSE stream disconnects mid-run, mast's session persists (per [`./durable-execution-design.md`](./durable-execution-design.md)); mast allows reconnect via the same `threadID` + `runID` and resumes streaming from the last durable event.~~ **Corrected 2026-09-17 (#98): today a disconnect does the opposite — it destroys the run.** The stream ctx descends from `r.Context()`, so a TCP reset cancels the turn. The written rationale for cancelling covers a *stalled* consumer — `emit` runs on the turn goroutine holding the per-session lock and the drain bracket, so a vanished reader must not pin the turn — and that hazard is real, but the remedy conflates *this consumer is gone* with *this work should stop*, and nobody recorded the second as a decision; it fell out of which context was in scope.

  It is measurably harmful and not only untidy: if the disconnect lands while a mutating tool is executing — after the effect, before its `FunctionResponse` persists — the session keeps no trace of the call, so a **well-behaved** client retry re-applies the change. Measured at 1 call vs. 2 on identical harnesses differing only in cancel timing.
  ...
  - **The correctness half — [#383](https://github.com/go-steer/mast/issues/383), pre-v1.0.** Subscriber-scoped cancellation: run the turn on a context that does not die with the request, and let a failed frame write drop the writer. Mostly the *removal* of a coupling.
...
**Corrected 2026-09-17 (#98).** The serialization is real, but it is a side effect rather than a policy: a second run blocks on the shared per-session turn lock inside `runTurnPre`. There is **no bundle key**, no queue depth, no refusal and no metric. The queue is unbounded and the only ceiling is `budget.max_wallclock_seconds`, so a caller queued behind a long turn gets a timeout that says nothing about why it waited. See [OQ #4](#open-questions) and [#384](https://github.com/go-steer/mast/issues/384).
