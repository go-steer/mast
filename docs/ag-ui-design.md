...
- **Deferred (documented, follow-on stages).** The `agui://` federation client ([Mast as AG-UI client](#mast-as-ag-ui-client)); client-declared tools (`RunAgentInput.Tools`, OQ #6 — note the field is parsed today and then dropped, never reaching `RunInput`); activity events (`StepStarted`, `ActivitySnapshot`) — coupled with reasoning in #98's issue text, but split out when Stage 4 shipped, because they are a different vocabulary with no sensitivity argument and their planner-step half needs a seam the emitter does not have yet; the mast-extension webhook push; and client-disconnect reconnect.
