...
**Implementation gate (spike-first, per the repo's verified-behavior discipline).** Three substrate behaviors are assumed above and must be verified before code lands: (a) `FunctionCall` events are durably appended before the tool executes — per-event persistence is proven for non-partial events, but the ordering relative to `tool.Run` is not; (b) what a resumed or continued turn actually does with a dangling unpaired `FunctionCall` in history (provider orphan error vs. model re-issue — either is survivable under this design, but the answer decides how loudly ambiguous-effect mode surfaces); (c) the runner plugin's tool callbacks fire for MCP-toolset tools, not just function tools. Metric families (effects replayed / ambiguous refusals — and the #50 marker-failure counter) are named in the v0.2 fixed-registry pass ([`./observability-design.md`](./observability-design.md)), not here.
...
8. ~~**Recorded-effect outbox shape (v0.2).**~~ *Resolved 2026-08-01 — see "Recorded-effect outbox" under "Side-effect semantics": the session log is the outbox (durable `FunctionCall` = intent, `FunctionResponse` = completion, retention inherited — no side table), checked from a process-wide runner-plugin interception layer shared with the permission gate's runtime wiring, with a fail-closed ambiguous-effect mode scoped to the mutation predicate. Residuals: the three verification items listed there gate the implementation, and the metric-family names belong to the observability fixed-registry pass.*
