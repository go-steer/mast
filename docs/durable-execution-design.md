...
1. **Drain, bounded by the turn's own ceiling.** On signal the daemon stops accepting inject work and waits for in-flight turns (inject-handler and attach-driven alike). The drain bound is the workload's `budget.max_wallclock_seconds` — a finishing turn is never cut shorter than its own budget allows — or 30s when no budget is set (an unbounded drain just invites supervisor SIGKILL mid-teardown). Consequence for deploys: the worst-case drain is known statically; size `terminationGracePeriodSeconds` / `TimeoutStopSec` above it (the K8s base and the systemd unit in the tree do).
2. **Pre-mark, then wait, then clear.** *Before* the drain waits, every session with a turn in flight gets a durable interruption marker appended (a `StateDelta` event, same mechanism as the abort marker) — so a SIGKILL landing mid-drain finds the markers already on disk. A turn that completes inside the window clears its marker; the log then honestly records a completed turn. Turns that outrun the window keep the marker and have their contexts cancelled just before exit. Like abort, the marker is state, not preemption: the engine ignores it, and a later turn on the session proceeds normally under reconstruct-and-re-execute.
