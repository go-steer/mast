...
4. **Resume identity is purely (session, interrupt/call ID); ADK has no token concept anywhere.** A resume turn is a user message carrying a `FunctionResponse` whose ID matches an open interrupt. Open interrupts survive unrelated user turns untouched (a fresh invocation runs alongside; the pending call stays answerable later), and replaying an already-answered interrupt is absorbed (`ErrNothingToResume`).
...
- Resume: the token resolves to (session, interrupt ID) and replays through the *existing* turn-locked, budget-wrapped resume path — the wire shape is unchanged, only the keying is new.
...
Sketch-to-plane mapping: watchdog pause-for-review = plane B with `Interrupt: true`; cost-cool-down = plane B from the budget-trip hook (or plane A if the planner chooses); ambiguity pause = plane A; maintenance-window / rate-limit backoff = either plane with `resume_at`; A2A `a2a_task_pending` = plane A (the outbound call site is a tool body).
...
- **Record shape:** `{token, plane, interrupt_id?, scope: {app, user}, reason, message, minted_at, expires_at, resume_at?}`. Plane-A records live in a small JSON map under one ops key (a session can hold several outstanding interrupt pauses); plane B is single per session — a second gate pause updates reason/message in place, audit-logged (open question #5 ratified: pause is single-writer per session, intra-session dupes are impossible).
...
- **Consumption + idempotency:** a successful resume consumes the token record (removed from the map, audit event appended). A second resume with the same token gets a structured `already_resumed` no-op. Plane-A replays racing past consumption are additionally absorbed by ADK's `ErrNothingToResume` (fact 4).
- **Resolution:** token → session is a reverse lookup. The v0.2 daemon builds an in-memory token index at boot (one ops-row scan, shared with the timed-pause scheduler's seed scan and #41's auto-resume scan) and maintains it on mint/consume — so `resume --token` needs the token alone, preserving the contract stated under "Resume triggers". The CLI accepts an optional session ID for `--session-db` direct mode (no daemon, no index). Fleet-scale indexing belongs to the P1.3 eventlog/query surface, same note as `list --state=paused`.

### Timed pause: the single-instance scheduler

`resume_at` rides the pause record on either plane. The daemon runs one scheduler goroutine over a min-heap keyed by `resume_at`, seeded from the boot scan, pushed on mint. Firing goes through the same doors an operator would use — no privileged side path: plane B → clear the gate marker (audited `timed-resume` ops event); plane A → synthesize the resume `FunctionResponse` (`{"resumed_by": "timer", "resume_at": ...}`) through the turn-locked, **budget-wrapped** resume path — resume turns are not budget-exempt (the #47 precedent), so a timed resume cannot smuggle spend past the meter, and a session that trips its budget on the timer fire pauses right back on the budget path. A timed resume consumes the token like any resume; an operator who resumes *earlier* than `resume_at` wins the race benignly — the scheduler finds the token already consumed and no-ops. Timers that expire while the daemon is down fire from the boot scan. Single-instance in v0.2 per the phasing table; multi-replica coordination stays v0.3.
...
Ordering: marker first, then cancel. Any turn that starts after the marker write sees it in the chokepoint check (which runs under the turn lock); the turn in flight is cancelled; there is no window where a new turn slips between marker and cancel unseen. Un-abort remains out of scope, unchanged.
...
- **`--pause-sessions`:** before draining, gate-pause (plane B) every session that is about to receive an interruption marker — i.e. the in-flight set. At next boot those sessions report `paused` (paused outranks interrupted), so #41's auto-resume — whose candidates are `interrupted` sessions only — skips them. This is the issue's "a planned stop is closer to pausing the daemon's sessions" made concrete: stop-and-hand-back-to-operator is `--pause-sessions`; stop-and-let-boot-repair-continue is the default.
...
Transcript states are untouched: gate-paused sessions project the existing `paused` state (reason/message in the pending column), precedence unchanged (`aborted > paused > interrupted > idle`) — which is exactly what makes `--pause-sessions` compose with #41 for free.
...
- **#41 auto-resume:** candidates are `interrupted` sessions only — gate-paused and aborted sessions are skipped by the state ladder; the boot ops-row scan is shared between the token index, the timer heap, and #41's candidate scan.
