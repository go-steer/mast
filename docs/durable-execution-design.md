...
*(Shipped 2026-08-04, `cmd/mast/autoresume.go` + `pkg/transcript.ScanInterrupted` + `pkg/effects.ScanDangling`.)* On boot the daemon runs one sequential goroutine (on `turnCtx`, so a shutdown drain cancels it) that scans for `interrupted` sessions and drives a continuation turn for each eligible one through the **same `runTurnPre` chokepoint** every other turn kind uses — inheriting the per-session turn lock, the budget meter, the tracker bracketing, and the effects outbox's fail-closed backstop. It is on by default (`--auto-resume`, `--auto-resume=false` to disable) and daemon-only: library consumers own their process lifecycle, the same rationale as "no `mast.Stop`".
