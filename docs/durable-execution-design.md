...
*(Shipped 2026-08-01, `pkg/effects` — the design above held, with three implementation refinements worth recording. (1) **A third class, `Spawning`, joined the predicate:** `invoke_specialist` and the `run_shape_*` vocabulary start sub-runs whose inner tool calls this process cannot individually guard (the planner dispatch runner is a separate in-memory-session runner — the known sub-runner bypass debt), so ambiguous-effect mode refuses them alongside mutating calls; they carry no records of their own. `invoke_remote_agent` classifies plain `Mutating` — remote effects are invisible, period. (2) **The predicate's MCP clause degrades to default-deny in practice:** ADK v2.1.0's `mcptoolset` drops tool annotations entirely at conversion (`convertTool` copies name/description/schemas only), so `readOnlyHint` never reaches mast and every MCP tool classifies mutating until the workload's `tool_catalog.tools` override un-gates it (`mutating: false`, audit-logged at startup; new per-tool policy list added to the bundle schema for exactly this). Upstream affordance — surfacing annotations through the toolset — is a candidate ask alongside adk-go#1229. (3) **The ack surface:** `mast sessions resume --ack-effects` (daemon-side, watermark written durably before the turn starts) and `mast.AckEffects` (library twin); the watermark rides the companion ops row (`mast_effects_ack_time/reason`), covers only intents persisted at or before it, and is deliberately not a transcript state — `list`/`show` derivation ignores it. Verification (a) is pinned end-to-end in the suite: a tool asserts its own FunctionCall is readable through a fresh store handle before it runs. Control calls (`adk_request_input`/`adk_request_credential`/`adk_request_confirmation`/`finish_task`) and long-running calls (`LongRunningToolIDs`) are excluded from the dangling scan — a paused HITL session's pending interrupt IS an unpaired FunctionCall and must not read as a dangling effect.)*
