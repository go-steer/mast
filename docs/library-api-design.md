...
> **Superseded 2026-09-14 by [`./compatibility-policy.md`](./compatibility-policy.md)** ([#304](https://github.com/go-steer/mast/issues/304)). That sentence was the right shape and it was one sentence: it did not say how long a cycle is, how a deprecation is announced, what counts as breaking, how long a release is supported, or what a "migration doc" is. The policy answers each — two released minors *and* 90 days; four announcement channels with the godoc marker's removal version enforced by a test; an enumerated breaking list including the two categories nothing here implies (a default that changes behaviour, and validation getting stricter on a file that used to load); the current release only, with no backports; and the CHANGELOG entry *is* the migration document. "Stable-marked" is also no longer how coverage is expressed — see the superseded-table note above.
...
6. ~~**Deprecation cycle length.** How many minor versions between deprecation announcement and removal? Bias: 2 minor versions (announce in v0.N, remove in v0.N+2) for pre-1.0; 1 major version for post-1.0.~~ **Resolved 2026-09-14 in [`./compatibility-policy.md`](./compatibility-policy.md)** ([#304](https://github.com/go-steer/mast/issues/304)) — mostly against this bias. The pre-1.0 half is moot: v1.0 is the next milestone and nothing will run a cycle before it. The count survives (two released minors) but **the bias's real error was answering in minors at all** — mast's fastest minor-to-minor gap turned out to be three days, so "2 minors" can be under a week. The policy requires two minors *and* 90 days, both.
