...
**Tool confirmation is a *second* resume path, and it is authorship-sensitive (verified 2026-08-15 against v2.2.0, building W7.0).** The write gate's approvals do not go through `ResumeOrRequestInput`; they go through `llminternal.RequestConfirmationRequestProcessor`, a default request processor on every LlmAgent flow. It scans session events backwards for the most recent event **authored `"user"`** and **bails entirely if that event carries no `FunctionResponse`** — otherwise it finds the `adk_request_confirmation` call, recovers the original call via `toolconfirmation.OriginalCallFrom`, skips calls already answered after the confirmation, and re-dispatches through `handleFunctionCalls`. Authorship is derived from content role, not set by the emitter (`agent.getAuthorForEvent`: `Content.Role == genai.RoleUser` → author `"user"`, everything else → the running agent's name). **Consequence for anything that emits its own events mid-turn:** an event you write with `genai.RoleUser` after the operator's confirmation silently disables the resume — the approved call never fires, the run ends idle, and nothing anywhere reports an error. Emit `genai.RoleModel`; a foreign-authored event still reaches another agent as user-role text via `ConvertForeignEvent` (`"For context: [<author>] said: …"`), so nothing is lost by doing so. See [`./spike-findings.md`](./spike-findings.md) "Later additions".
...
## v2.2.0 (pin moved 2026-08-13)

The pin is now `google.golang.org/adk/v2 v2.2.0`, taken ahead of the v0.3 workstreams that build on the code it changes. Everything marked *verified against v2.1.0* above was re-checked against the v2.2.0 diff and still holds — in particular `workflow/parallel_worker.go` (the parallel-branch invisibility finding) and `internal/llminternal/outputschema_processor.go` are byte-identical between the two. What moved:

- **A cancelled graph run no longer reports success** (`workflow/scheduler.go`). v2.1.0 reacted to invocation-context cancellation only in its `doneChan` select arm, and a ready `doneChan` need not win against a queued node completion; `cancelAll` cancels per-node contexts and never touches `parentCtx`, so nothing else caught it. v2.2.0 records the cause, drains, and surfaces it — with a node's own error still outranking it, so "context canceled" can't mask the real reason a run stopped. `NodeCancelled` now covers external cancellation too, and the two are told apart by whether the run reports an error, not by the status. Pinned by `TestGraphRunSurfacesExternalCancellation` in `pkg/graph`, verified to fail on v2.1.0. This is durability-relevant for every mast path that cancels a turn from outside the graph.
- **Confirmation resume is deterministic and no longer double-dispatches** (`internal/llminternal/request_confirmation_processor.go`). Re-dispatch follows the order the confirmation requests appear in the event rather than Go's randomized map iteration (which made the resumed calls, the assembled response, and the last-writer-wins `StateDelta` merge vary run to run); two confirmations resolving to the same original call ID now dispatch once; and a call whose response is already in the event snapshot is skipped rather than re-run. Relevant substrate for the v0.3 approval work ([`./v0.3-plan.md`](./v0.3-plan.md) W2).
- **`memory.InMemoryService.Search` holds its read lock across the scan** — it previously released before iterating and could panic against a concurrent `AddSessionToMemory`.
- **`session.Event`, `EventActions`, and `model.LLMResponse` carry JSON tags** — PascalCase to camelCase with `omitempty`, plus a custom `EventActions.MarshalJSON` (shadowed pointer map fields keep nil distinguishable from empty) and `Event.UnmarshalJSON` (accepts numeric epoch-second timestamps, strips all-zero `NodeInfo`), for adk-python interop. Decoding is unaffected in both directions because `encoding/json` matches case-insensitively; the exposure is external readers of the attach `agent` frame, which embeds the struct verbatim (see `pkg/attach/events.go`).
- **Live/bidi streaming fixes** (`internal/llminternal/base_flow.go`) — producers guard sends on the connection context, and a failed `SendHistory` now runs cleanup instead of stranding the websocket. mast does not use live mode; noted so the diff isn't re-read later.
- **Streaming usage metadata is no longer clobbered** by a later chunk with nil `UsageMetadata` (`stream_aggregator.go`). Does not bite today — mast runs `StreamingModeNone` everywhere — but it is a prerequisite for correct cost attribution if that changes.
- **Documentation only:** `LlmAgent.OutputSchema`'s comment now says the framework injects a `set_model_response` tool so the model can still call tools. The processor implementing that shipped in v2.1.0 unchanged; the old comment was simply stale.

Unmoved: `session/database` is untouched, so [adk-go#1229](https://github.com/google/adk-go/issues/1229) (the concurrent-append affordance, tracked in mast #51) is still open and the ops-row convention stays.

The bump pulls direct dependencies ADK's own `go.mod` requires: MCP go-sdk 1.4.1 → 1.7.0, genai 1.63 → 1.66, glebarez/sqlite 1.8.0 → 1.11.0, jsonschema-go 0.4.2 → 0.4.3, gorm 1.31.0 → 1.31.2, grpc 1.82.1 → 1.83.0, plus otel/api/genproto. **MCP 1.7.0 enables SEP-2322 multi-round-trip client middleware by default**, which auto-fulfills server-initiated input requests and retries the call; none of that traverses mast's approval gate, which sits on tool dispatch rather than on a round trip inside a call already in flight. *Closed in #176 — and the fix was not the one this paragraph originally proposed.* `MultiRoundTripOptions.Disabled` turns off the client-side middleware, but that middleware only runs on protocol ≥ 2026-07-28, and `StreamableServerTransport.SupportsProtocolVersion` serves that version only when the server is **stateless**; an ordinary stateful HTTP server negotiates 2025-11-25 and its own server-side middleware sends mast a real `elicitation/create` / `sampling/createMessage` / `roots/list` request instead. Nor was the gap latent: this paragraph's original claim that mast "registers no elicitation or sampling handler, so such a request errors today" holds for those two and not for roots, which the SDK answers itself from the client's own root set with no handler and no capability opt-in. Measured pre-fix against a stateful fixture, one approved dispatch ran the server's tool **20 times**. `pkg/mcp.newMCPClient` therefore does three things — disables the middleware, advertises no client capabilities, and refuses all three methods in a receiving middleware — and `pkg/mcp/mrtr_test.go` pins each regime separately.
