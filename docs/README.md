...
| Docs site = **Astro ^7 + Starlight** (core-agent's actual `docs/site` convention; the corpus's "Hugo"/"Hugo + Docsy" references were stale — corrected 2026-07-26); v0.1 skeleton shipped at `docs/site/` (landing + install + 3 quickstarts + reference + roadmap), build-only CI (`ci-docs.yml` via `dev/tools/docs-site.sh`), deploy deferred until the repo is public; house rule #4 (walk design docs + site together) is now active | [`./fork-design.md`](./fork-design.md) Phase 0/Phase 4 + `AGENTS.md` house rule #4 |
