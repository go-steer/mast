...
*(Shipped v0.10, 2026-09-21 — a model call that met a transient provider rejection (#452). The dispatch family above counts what a rejection did to a delegation; this one counts the rejection itself, one tier down, for every model call in the process rather than only the ones under a specialist. It exists because mast's own evals have carried a retry since 2026-08-21 — three of thirty-one judge rows were lost to a Vertex 429 that night — which left mast's measurement of mast resilient to a shed while the product was not:*

- *`mast_provider_retries_total{workload, outcome}` — one increment per model call that met a retryable rejection (HTTP 429 or 503, classified off the provider's own error type and never off message text); outcome ∈ `recovered` (the bounded retry waited and the second attempt returned content) / `exhausted` (the schedule ran out and the provider's error is what the caller got) / `declined` (the process-wide cooldown refused the retry because another call had just taken the window)*

*A call that met no rejection increments nothing, and neither does one that failed for any other reason — a 400 is not provider pressure and does not belong in this family's denominator. `recovered` is the series that justifies the family: it is the only trace a successful-but-shed call leaves anywhere, since the turn it belongs to completes, returns content, and looks unremarkable except for being two seconds slower. `declined` reads differently from the other two — it is mast reporting on itself rather than on the provider, and a sustained `declined` rate beside `mast_dispatches_total{outcome="rate_limited"}` says the fan-out is wider than the quota, which no amount of retrying fixes. Labelled by workload and not by model or provider: the model name is workload-authored and unbounded, so it goes in the `WARN` line the reporter also emits — the same reasoning that kept `specialist` out of the dispatch family.)*
