...
- **GitHub Releases** for each tag: `mast-linux-amd64`, `mast-linux-arm64`, `mast-darwin-amd64`, `mast-darwin-arm64`, `mast-windows-amd64.exe`. Signed release artifacts (cosign). *What ships is `mast_<version>_<os>_<arch>.tar.gz` for linux/darwin x amd64/arm64 — no Windows build and no bare binaries — plus `checksums.txt`, its signature since 2026-09-19, and a build provenance attestation per asset since 2026-09-20. See below.*
...
### Build provenance — shipped 2026-09-20 ([#342](https://github.com/go-steer/mast/issues/342))

Every release asset, the container image and the Helm chart now carry a [SLSA v1](https://slsa.dev/spec/v1.0/provenance) build provenance attestation, generated by `actions/attest-build-provenance` and verified — against the published artifact, not the runner's copy — by [`dev/release/verify-provenance.sh`](../dev/release/verify-provenance.sh). Four decisions.

**Provenance is not the signature restated.** It is tempting to read "signed and attested" as belt and braces over one claim, and it is two claims. `checksums.txt.sig` answers *who signed this*: mast's release workflow, at this tag. The attestation answers *what built it* — source repository, commit SHA, workflow file and ref, runner environment, invocation. Only the second lets a reader connect a tarball in their hand to the tree they are reading on GitHub, and it is the question that gets asked after a CVE advisory rather than before an install.

**Every asset, where signing deliberately covers one.** The signature covers `checksums.txt` alone because that file transitively covers the tarballs and a verifier should have exactly one thing to check. Provenance goes the other way: it is attested per subject, all four archives plus the checksum file, because `gh attestation verify mast_X_linux_amd64.tar.gz` is the command someone runs against the file they downloaded, and routing that through a checksum list would reintroduce the hop the per-subject form removes. The two artifacts answer different questions and the cost profile differs — a second signature says nothing new, a second subject does.

**The generated predicate, not a hand-written one.** `cosign attest` with a predicate this repo composes would put mast in charge of describing its own build from values its own workflow chooses, which is precisely the field `gh attestation verify` documents as forgeable by a compromised workflow. The action populates the certificate from the GitHub OIDC token and the runner's context instead, so the fields a verifier pins are ones the build cannot write.

**`--repo` alone is this feature's `.*`.** The wildcard finding from signing recurs here in a different costume: `gh attestation verify <file> --repo go-steer/mast` succeeds for an attestation minted by *any* workflow in this repository that holds `attestations: write` — the docs build would do. The verifier that means anything pins `--signer-workflow` (which workflow file) and, for the release assets, `--source-ref` (which tag, rather than somebody's branch). The script writes all of them so no reader has to choose, the same reason `verify-signature.sh` writes the exact cosign identity. Measured rather than assumed in the direction that matters: run against v0.9.0, which predates attestation, the script fetches all five assets, gets a 404 per digest and exits 1 — the check can fail.
