...
**What it deliberately is not.** It is not leader election and it does not make mast multi-replica. A replica that loses the race at boot stays passive for its whole life: it does not watch the lease, and it does not take over when the leader dies later. Kubernetes restores the leader instead — the *restarted* process reclaims the lease once it goes stale, and the StatefulSet restarts the pod that held it. Takeover by a live passive replica is a genuine feature and is not this one; it needs session-ownership handoff (above) to be real first, or two replicas will hand the *lease* over while still both believing they own the sessions.
