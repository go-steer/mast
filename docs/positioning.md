...
- **Skills subsystem** (`pkg/skills/` + ADK `skilltoolset`). Cut from mast's scope. The "callable task template" use case is replaced by the specialists subsystem (subagent-as-tool with budget + tool allowlist; see `./specialists-design.md`). The Anthropic-SKILL.md-compat shape stays in core-agent for its audience.
