...
**On "library-embedded" specifically:** it names a *capability* mast commits to (few agent frameworks are cleanly embeddable), not a deployment mode that excludes the standalone binary. Mast ships **both** consumer shapes as equal first-class citizens — a `mast` binary for Cloud Run / GKE / systemd / laptop CLI use, and a Go library (`import "github.com/go-steer/mast/..."`) for host services that want agent capabilities inline. What is genuinely the same is the **governance layer**: the agent loop, the dispatch shapes, the write gate, the effect outbox, budget ceilings and the watchdog are one implementation, and durability is the same session event log. *"Only the config-injection surface differs"* was the wording here since before the fork and it was an overclaim, corrected 2026-09-11 ([#288](https://github.com/go-steer/mast/issues/288)): everything that starts a turn without a caller — schedules, the monitoring cycle, notify, auto-resume, timed pauses, drain — plus every operator listener is `package main` under `cmd/mast` and an embedder does not get it. That is a defensible split, because a host service brings its own trigger and its own HTTP surface, but it is a split. [`./deployment-design.md`](./deployment-design.md) enumerates all four production topologies (Cloud Run, GKE, library-embedded, standalone) and [`./library-api-design.md`](./library-api-design.md) covers the library-consumer contract.
...
4. **HA for scheduled workloads.** ~~`cmd/mast/schedtrigger.go` is single-instance by the single-writer rule, and says so: two replicas of a scheduled workload each keep their own cadence and both fire. Note what does and does not exist — `pkg/eventlog/lock.go` holds a real heartbeat lease (`agent_run_lock`) that stops two processes from running *the same session*, so the primitive is there at the session grain and nothing uses it at the fleet grain. Jitter is a collision softener, not an election. An operator who reads "durable" as the fourth pillar and then runs two replicas gets each schedule fired twice, which is the kind of surprise an installed product does not get to hand out.~~ **Half closed, 2026-09-18.** The lease the finding said existed at the session grain and went unused at the fleet grain is now used at both: one instance drives the schedules, the timed resumes and the boot auto-resume, and every other one refuses out loud. The surprise is gone. The *HA* is not — a passive replica never takes over, so scheduled work still runs on one pod and still pauses while that pod is replaced, which needs session-ownership handoff to fix. Tracked as [#345](https://github.com/go-steer/mast/issues/345) (closed) and [#403](https://github.com/go-steer/mast/issues/403) (the takeover half, open), with the mechanism in [`./deployment-design.md`](./deployment-design.md) § "The scheduling lease".
...
- **Specialists subsystem** (`pkg/specialists/`) — subagent-as-tool pattern using ADK's `agenttool`. `.specialist.md` files with YAML frontmatter define specialists with budgets, model overrides, and tool allowlists. Replaces the skills surface from core-agent's scope. See `./specialists-design.md`.
- **Multi-provider abstraction** (`pkg/providers/`): **two vendors, four deployment paths** — Gemini and Claude, each first-party or on Vertex. The moat is the *substitutability*, not the count: a specialist declares `tier: small | mid | frontier` and the same bundle runs on either vendor with no code change, the meter prices the (backend, model) pair, and a judged corpus runs nightly against both so the equivalence is measured. This line used to read "Gemini, Vertex, Anthropic, Anthropic-Vertex, echo, scripted", which counted one vendor's deployment surfaces and two test doubles as peers of real backends — `mock` is a fake and `vertexcache` is a cache in front of one of the two (corrected 2026-09-11, [#303](https://github.com/go-steer/mast/issues/303)). Most agent frameworks reach two vendors, so *two* is not the differentiator and should not be stated as one. The third backend ([#312](https://github.com/go-steer/mast/issues/312), post-v1.0) earns the plural and is the first honest test of whether `tier:` survives a vendor the abstraction was not designed against.
...
1. **README + docs-site repositioning sweep** *(Astro + Starlight; skeleton shipped 2026-07-26)*. First impression matters most. Lead with unattended / platform story. Cheap, immediate, sets the frame for everything else.
