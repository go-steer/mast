...
The server lands in stages so the read/control surface and its auth ship — and get exercised end-to-end — before turn execution:

- **Stage A (shipped).** Agent-card publication (aggregated + per-workload), `tasks/get`, `tasks/cancel`, and pluggable bearer auth (`a2a.TokenValidator`, built-in `StaticBearerValidator`) with per-skill scope checks. The server runs on its own listener (`--a2a-listen`), separate from the inject and attach surfaces; card endpoints are public, `/a2a` is authenticated when a validator is configured. `tasks/get` maps the session's log-proven state onto the A2A lifecycle and **never reports `completed`** from a transcript-only read (the event log cannot prove a turn finished vs. is in flight) — `completed` arrives with the in-process task registry in Stage B. Build-vs-buy: hand-rolled over the wire types this repo already owns, so every A2A task runs through the same `runTurnPre` chokepoint every other turn kind funnels through (budget, pause, abort, turn-lock, effects outbox by construction) — ADK's `adka2a.Executor` drives the runner directly and bypasses it.
- **Stage B1 (shipped).** Synchronous `message/send` through `runTurnPre` (task ID == session ID). The turn drives the same chokepoint as every other kind; a per-turn `onEvent` capture records the model's final answer, which surfaces as a `result` text artifact on the terminal task. The **in-process task registry** is the authority for `completed`/`failed` (which the transcript cannot prove): `GetTask` consults it first and falls back to the transcript projection for tasks this process did not run (e.g. after a restart) or that are still in flight / non-terminal (working / input-required — the transcript is authoritative there, so the registry pins only terminal outcomes). The registry write is **cancel-wins**: once `CancelTask` records `canceled`, a racing `message/send` that finalizes as `completed` cannot resurrect the record — so a canceled task never leaks the model's answer as a result artifact, regardless of which write lands last.
  - **Task-id ownership fencing.** The A2A surface addresses only tasks it minted — ids carrying the `a2a-` prefix from the server's `mintTaskID`. `GetTask` / `CancelTask` / `message/send`-continuation all refuse (`-32001` TaskNotFound) any other session id, so a scope-holding caller cannot read, cancel, or drive a turn into a session owned by another surface (inject `incident-*`, attach, autoresume) by presenting that session's id as a task id.
  - **contextId.** A fresh send with no `contextId` gets a server-minted one (`ctx-` prefix), returned on the task so the caller can group follow-up messages; a continuation *that omits it* inherits its task's contextId from the in-process registry. This matches A2A v0.3's "server assigns contextId" rule. Server-side inheritance is a convenience for still-tracked tasks only: the contextId is not durable, so a client that omits it on a continuation after the record has cleared (a HITL park, a cancel, or a restart) receives a freshly minted one. Per the A2A spec a client echoes the contextId it was given, so a compliant client is unaffected — grouping stability is the client's responsibility, not a server guarantee.
  - **Ingest shape.** Text-only for now (data/file parts are rejected with `-32602`); a fresh message routes to the single exposed skill (an endpoint exposing more than one skill refuses a fresh, selector-less `message/send` with `-32004` — multi-skill routing by message metadata is a later stage). Note the mast A2A **client** sends structured inputs as a `data` part (the federation calling convention), which this text-only server rejects — so there is no mast↔mast `message/send` round trip yet; the server's counterpart for structured ingest is a later stage.
  - **Trace propagation** is wired both directions: the client injects the caller's W3C trace context (`traceparent`/`baggage`) on every outbound JSON-RPC call, and the server extracts it so the turn's spans parent under the caller's span (no-op when tracing is disabled).
  - The drain gate refuses new tasks with `a2a.ErrUnavailable` once shutdown begins (mirroring the inject door), which the server maps to the retryable server-error code `-32000` rather than an internal fault.
- **Stage B2 (rate limiting shipped; tenant isolation deferred).** Per-caller / per-workload rate limiting via a pluggable `RateLimiter` seam on `a2a.Config` (designed for AG-UI to reuse, #11) — shipped; see [Rate limiting](#rate-limiting) below. The tenant-claim (`Principal.Tenant`) → session-isolation half stays **deferred**: ADK v2.1.0's `IsolationScope` is an *event/task-level* field (the workflow `finish_task` machinery — `runner.findActiveTaskIsolationScope`), not a session-create or tenant seam (`session.CreateRequest` carries `{AppName, UserID, SessionID, State}`, no scope), and there is no `WithIsolationScope` runner/session hook. Multi-tenant session isolation therefore waits on either an upstream ADK session-scope seam or a mast-side user-namespacing design (see [Multi-tenancy](#request--session-mapping) below). `Principal.Tenant` ships now as the rate-limiter's caller identity.
- **Stage C (shipped).** `message/stream` over Server-Sent Events. The endpoint runs the same `runTurnPre` turn as `message/send` — send and stream share one turn-drive body (`a2aBackend.runTask`), differing only in whether interim progress is emitted — so the chokepoint, rate limiter, and ownership fence all apply identically. Frame sequence per stream: an initial `Task` snapshot (`working`), then a `TaskStatusUpdateEvent` (`final:false`) per model-authored event carrying its text as progress, then the closing bookend the server builds from the terminal snapshot — a `TaskArtifactUpdateEvent` with the agent's answer (when non-empty) and a final `TaskStatusUpdateEvent` (`final:true`) carrying the terminal state. Each frame is one JSON-RPC response in an SSE `data:` block. Two decisions worth recording:
  - **Message-granular, not token deltas — a property of the frame, not of the turn mode** *(settled #408, 2026-09-19; the earlier wording derived this from `StreamingModeNone` and was wrong).* A `TaskStatusUpdateEvent` carries a whole `a2a.Message` with its own `messageId` and no way to mark it a continuation of the previous one, so a fragment has no honest encoding in it: an N-chunk answer emitted frame-per-chunk arrives as N distinct agent messages, not as one being built. A2A *does* have a delta mechanism — `Append` + `LastChunk` — and put it on `TaskArtifactUpdateEvent`, which is where token-level narration would have to land if mast ever wants it. So Stage C is message-granular by protocol, and `emitStreamProgress` skips partial events (the same one-line `ev.Partial` guard as the watchdog bridge and AG-UI, #400/#401) rather than relying on the turn loop never producing any. ADK's contract is that the Runner fully processes only the final non-partial event, so this yields the same frames under `StreamingModeSSE` as under today's `StreamingModeNone`. This matches the [open question](#open-questions) bias toward terminal-state-only for the client direction.
  - **Reuses the SSE *framing pattern*, not a broadcaster.** The earlier note said "reusing the attach broadcaster"; a broadcaster is fan-out to N subscribers, but `message/stream` is one caller = one turn = one stream. The existing synchronous, in-order `onEvent` hook already delivers events on the handler goroutine, so Stage C reuses attach's `text/event-stream` + `http.Flusher` framing without any fan-out goroutine or locking. Auth, scope, and **rate-limit** refusals are decided *before* the SSE upgrade (the upgrade is lazy, on the first emitted frame), so a refusal rides a normal JSON-RPC error rather than a truncated stream; `message/stream` shares `message/send`'s `(caller, workload)` rate-limit bucket. The card advertises `capabilities.streaming: true`.
  - **The final answer rides two channels.** The last complete model event *is* the answer, so it appears both as the last progress `status-update` (live narration) and as the closing `artifact-update` (the deliverable). These are distinct A2A channels, and the duplication is inherent to narrating on the status channel at all — it does not go away under a streaming turn mode, because the last non-partial event is still both the final narration frame and the deliverable. It would only go away by moving narration to `Append`/`LastChunk` artifact deltas. Progress frames each get a distinct `messageId` (a per-frame sequence) so a client that dedupes by it does not collapse a multi-round turn.
  - **`turnCapture` deliberately has no partial guard.** It takes last-wins over model-authored text, and the complete response arrives last in every ADK mode, so it picks the aggregate without one. Adding a guard there "for consistency" would empty the result artifact on any provider that ends a turn on a partial event; a test pins the asymmetry.
  - **The stream always closes with `final:true`, even on a non-terminal projection.** The final frame carries the same state `message/send` would reply with — usually terminal or `input-required`, but a turn the chokepoint refused because the session was gate-paused (operator/timed, no pending interrupt) projects as `working`. The stream still ends (the turn returned; fabricating a terminal state would lie), and a client seeing `working` + `final` should poll `tasks/get` to follow the task as it resumes.
  - **A stalled consumer cannot pin a turn.** `emit` runs inside `runTurnPre` while the per-session turn lock and the drain in-flight bracket are held, so a frame write that blocks on a full socket buffer would otherwise survive drain-window cancellation. Each frame write carries a per-frame deadline, and a write failure (deadline lapsed or client gone) cancels the turn ctx so it aborts and releases the lock/bracket; the deadline is cleared before the handler returns so it cannot leak onto a reused keep-alive connection. Failed turns project a generic status to the external caller (the internal error goes to the log, not the wire).
...
- **Under the daemon's own validator the check cannot fail** *(added 2026-09-17, [#389](https://github.com/go-steer/mast/issues/389))*. `MAST_A2A_TOKEN` is a single token resolving to a single principal (`Subject: "mast-a2a-static"`) that holds the **union of every exposed skill's scopes**, because one token has to reach all of them. The check above runs; it has no reachable failing branch. So the three things per-skill scopes look like they buy you — a token that reaches one skill and not another, a meaningful `Subject` in the audit trail, a rate-limit bucket per caller — arrive only with a **host-supplied `TokenValidator`**, which is the seam this section already describes and which needs no change to the server. The daemon-side answer, a token→`{subject, scopes}` map rather than one env var, is [#389](https://github.com/go-steer/mast/issues/389) option 2; it is not freeze-exposed, since `MAST_A2A_TOKEN` is an environment variable and the v1.0 promise covers flags, verbs and exit codes. AG-UI has the identical construction and one extra edge — it *publishes* per-workload scopes on `/agui/agents.json` — see [`./ag-ui-design.md`](./ag-ui-design.md) § Auth.
...
External callers can consume mast's provider budget through A2A `message/send`. A pluggable `a2a.RateLimiter` seam gates admission (`Config.Limiter`; nil disables it) — the same extension point AG-UI is designed to reuse (#11). The server calls `Allow` once per **turn-driving** request (`message/send`) with a `RateLimitRequest{Subject, Tenant, Workload, Method}`; a refusal maps to the retryable `-32000` code with an advisory `Retry-After` header and a `rejected` task-outcome metric. **Control-plane verbs (`tasks/get`, `tasks/cancel`) are deliberately never gated** — an operator must always be able to read or cancel a task, and cancel is destructive-but-cheap. This is where rate limiting differs from the auth scope check, which *does* cover `tasks/cancel`.

The built-in `TokenBucketLimiter` keeps an independent token bucket per **(caller, workload)**, where the caller is the request's `Tenant` if set else its `Subject` (so a multi-token tenant is limited as one caller; an unauthenticated endpoint buckets all callers together). It admits only when a token is available immediately and reports the wait-until-next-token as `Retry-After` on a refusal, consuming no future capacity. The daemon builds it from `MAST_A2A_RATE` (requests/second per caller×workload) and `MAST_A2A_BURST` (bucket depth; defaults to `ceil(rate)`, min 1); `MAST_A2A_RATE` unset means no rate limiting, and a set-but-malformed value fails startup. The bucket map is unbounded at v0.2 single-instance scale (bounded eviction is a follow-on), consistent with the daemon's per-session pools.
...
**Tenant scope propagation (deferred).** An A2A token can carry a tenant claim (`tenant_id` or similar), surfaced as `Principal.Tenant`. The intent is to isolate a tenant's sessions from one another; the original design mapped this to `WithIsolationScope(tenantID)` on the session. That mapping is **not yet buildable on the pinned ADK (v2.1.0)**: `IsolationScope` there is an event/task-level field driven by the workflow `finish_task` machinery (`runner.findActiveTaskIsolationScope`), scoping which agent contexts see an event *within* a session — not a session-create or tenant seam (`session.CreateRequest` carries no scope field). Until an upstream session-scope seam lands (or mast adopts a `UserID`-namespacing design for tenant partitioning), `Principal.Tenant` is used only as the [rate limiter](#rate-limiting)'s caller identity, and tenant session isolation stays deferred.
...
| **Multi-tenant** | A2A auth token can carry a tenant claim (`Principal.Tenant`), used now as the [rate limiter](#rate-limiting)'s caller identity. Tenant *session isolation* is deferred — ADK v2.1.0's `IsolationScope` is event/task-level, not a session/tenant seam (see [Tenant scope propagation](#request--session-mapping)). Cross-tenant A2A calls require explicit opt-in (bundle field). |
...
6. ~~**Rate limiting on the A2A server side.**~~ *Resolved 2026-08-07 (Stage B2): shipped a pluggable `a2a.RateLimiter` seam + built-in per-(caller, workload) `TokenBucketLimiter`, gating `message/send` only (control-plane verbs stay ungated). See [Rate limiting](#rate-limiting). Per-workload concurrent-task caps and budget-per-caller remain extensions (follow-on).*
