...
- **Stage C (shipped).** `message/stream` over Server-Sent Events. The endpoint runs the same `runTurnPre` turn as `message/send` — send and stream share one turn-drive body (`a2aBackend.runTask`), differing only in whether interim progress is emitted — so the chokepoint, rate limiter, and ownership fence all apply identically. Frame sequence per stream: an initial `Task` snapshot (`working`), then a `TaskStatusUpdateEvent` (`final:false`) per model-authored event carrying its text as progress, then the closing bookend the server builds from the terminal snapshot — a `TaskArtifactUpdateEvent` with the agent's answer (when non-empty) and a final `TaskStatusUpdateEvent` (`final:true`) carrying the terminal state. Each frame is one JSON-RPC response in an SSE `data:` block. Two decisions worth recording:
  - **Message-granular, not token deltas.** The turn path runs `StreamingModeNone`, so `onEvent` yields whole model responses, not token streams — Stage C surfaces progress at message granularity (one status-update per model response). Token-level streaming would require switching the core turn loop to `StreamingModeSSE` for *every* turn kind and is a follow-on, not this stage. This matches the [open question](#open-questions) bias toward terminal-state-only for the client direction.
  ...
  - **The final answer rides two channels.** Because the turn runs `StreamingModeNone`, the last model event *is* the answer, so it appears both as the last progress `status-update` (live narration) and as the closing `artifact-update` (the deliverable). These are distinct A2A channels; the duplication is inherent to message-granular streaming and disappears once token-level deltas land. Progress frames each get a distinct `messageId` (a per-frame sequence) so a client that dedupes by it does not collapse a multi-round turn.
