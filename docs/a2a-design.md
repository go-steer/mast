...
- **Stage B2 (deferred).** Per-caller / per-workload rate limiting (a pluggable `RateLimiter` seam on `a2a.Config`, reused by AG-UI) and tenant-claim (`Principal.Tenant`) → `WithIsolationScope` on the session. Deferred pending an ADK session-create seam under `AutoCreateSession`.
