...
5. ~~**Interaction with plan-first gate.**~~ *Resolved 2026-08-12 (see [`./orchestration-design.md`](./orchestration-design.md) "Mutation approval: granularity, grants, and change sets"): plan-first stays a **root-agent primitive** and does not reach into graphs, confirming the stated bias — but the reason is now structural rather than aesthetic. The hazard plan-first would be reaching into a graph to prevent is unreviewed mutation, and that is covered at a different seam: `hitl_policy.on_mutation` gates at tool execution, which behaves identically regardless of the shape wrapped around it. A graph node cannot mutate past the gate whether or not a plan was recorded, so extending plan-first inward would add a second, weaker control over ground the mutation gate already holds. Plan-first gates "has the root agent thought about this yet"; the mutation gate gates "may this specific call happen." Note the two artifacts stay deliberately distinct: plan-first's `plan_it_out` is prose, while the mutation gate's `per_change_set` unit is a typed change set — see the same section.*
